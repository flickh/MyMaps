...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //for current location EXTENSION
        //checkMyPermission()
    ...
    // fun checkMyPermission(googleMap: GoogleMap) {
    ...
    private fun serializeUserMaps(context: Context, userMaps: List<UserMap>) {
        ObjectOutputStream(FileOutputStream(getDataFile(context))).use {
            it.writeObject(userMaps) //writes out some binary data, breakes it down into basic component
        }
    }

    private fun deserializeUserMaps(context: Context) : List<UserMap> {
        val dataFile = getDataFile(context)
        if (!dataFile.exists()) { //data file dne yet
            return emptyList()
        }
